...
```{r}
library(ggplot2)
ggplot(data=fatal_rate, aes(x=Year, y=Crude.Rate, group = Cause.of.Death, colour = as.factor(Cause.of.Death))) + geom_line() + labs(title = 'Rate of Homicide and Unintentioanl Deaths, 1981-2010', y = 'Per 100,000 people', colour= 'Cause of death')
```
### Reasoning
The original graph is misleading with the stand-alone line of firearm homicide deaths. It was hard to draw conclusions about reason behind the rate without comparing it to other death rate. In the revised version, the rate of firearm homicide deaths is compared with violent-related deaths (including all homicide and suicide) and unintentional deaths. From the revised graph we can tell that it is not just the firearm deaths that's dropping but also the overall voilent-related deaths over years. This point is confirmed by Ehrenfreund (2015) that the drop in gun violence is part of the drop in overall voilent crime. 

### Detour
1. Pulling data in a way that is similar to the original graph and easy for visualization
2. Determining the categories of cause of death so as to draw a comprehensive picture of death rate changes over years
...
(Link: https://public.tableau.com/profile/tianmin.li#!/vizhome/gun_suicide/Dashboard1)
...
2. weight sample data 
3. polish graph color and size, 
